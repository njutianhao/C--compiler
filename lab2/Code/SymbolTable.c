...
Type create_Basic_Type(char *ifint)
{
    ...
    if (strcmp(ifint, "int") == 0)
        temp->kind = BASIC_INT;
    else if (strcmp(ifint, "float") == 0)
        temp->kind = BASIC_FLOAT;
    else
    {
...
Type create_Structure_Type(FieldList head, char *struct_Name)
{
    ...
    if (struct_Name != NULL)
    {
        ...
        temp->u.structure.structName = NULL;
    return temp;
}
//创建function type类型
...
int check_return(char *funcName, Type returntype)
{
    ...
    if (same(functype->u.function.returnType, returntype) == 1)
        return 1;
    return 0;
}
//获取返回值类型
...
int same(Type A, Type B)
{
    ...
    case STRUCTURE:
    {
        if (A->u.structure.structName == NULL && B->u.structure.structName == NULL)
            return 0;
        if (strcmp(A->u.structure.structName, B->u.structure.structName) == 0)
            return 1;
        else
            return 0;
        break;
    ...
    case FUNCTION:
    {
        ...
        {
            if (paramB->type->kind != paramA->type->kind)
                return 0;
...
void printTable()
{
    ...
    for (int i = 0; i < TABLE_SIZE; i++)
    {
        struct TableNode *p = SymbolTable[i];
        while (p != NULL)
        {
            printf("hashnum:%d  ", i);
            if (p->name != NULL)
            ...
            else
            {
                ...
                {
                case BASIC_INT:
                    printf("type:BASIC_INT\n");
                    break;
                case BASIC_FLOAT:
                    printf("type:BASIC_FLOAT\n");
                    break;
                case ARRAY:
                {
                    printf("type:ARRAY\n");
                ...
                case STRUCTURE:
                {
                    printf("type:structure  structname:%s\n", p->type->u.structure.structName);
                    break;
                }
                case FUNCTION:
                {
