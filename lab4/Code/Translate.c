...
void translate_Exp(struct GrammarTree *node, Operand place)
{
    ...
    else if (first->type == Exp && get_child(node, 2)->type == ASSIGNOP)
    {
        ...
        else
        {
            if (op1->kind == OP_ADDRESS && op2->kind == OP_ADDRESS)
            {
                ...
                create_InterCode_twoOp(tmp, op2, IR_ASSIGN);
                create_InterCode_twoOp(op1, tmp, IR_ASSIGN);
            }
            else
                create_InterCode_twoOp(op1, op2, IR_ASSIGN);
        }
    ...
    else if (first->type == Exp && get_child(node, 2)->type == PLUS)
    {
        ...
        if (op1->kind != OP_ADDRESS && op2->kind != OP_ADDRESS)
            ...
            create_InterCode_threeOp(place, t1, t2, IR_ADD);
        else if (op1->kind == OP_ADDRESS)
            create_InterCode_threeOp(place, t1, op2, IR_ADD);
        else
            create_InterCode_threeOp(place, op1, t2, IR_ADD);
    }
    //Exp -> Exp MINUS Exp
    else if (first->type == Exp && get_child(node, 2)->type == SUB)
    {
        ...
        if (op1->kind != OP_ADDRESS && op2->kind != OP_ADDRESS)
            ...
            create_InterCode_threeOp(place, t1, t2, IR_SUB);
        else if (op1->kind == OP_ADDRESS)
            create_InterCode_threeOp(place, t1, op2, IR_SUB);
        else
            create_InterCode_threeOp(place, op1, t2, IR_SUB);
    }
    //Exp -> Exp STAR Exp
    else if (first->type == Exp && get_child(node, 2)->type == STAR)
    {
        ...
        if (op1->kind != OP_ADDRESS && op2->kind != OP_ADDRESS)
            ...
            create_InterCode_threeOp(place, t1, t2, IR_MUL);
        else if (op1->kind == OP_ADDRESS)
            create_InterCode_threeOp(place, t1, op2, IR_MUL);
        else
            create_InterCode_threeOp(place, op1, t2, IR_MUL);
    }
    //Exp -> Exp DIV Exp
    else if (first->type == Exp && get_child(node, 2)->type == DIV)
    {
        ...
        if (op1->kind != OP_ADDRESS && op2->kind != OP_ADDRESS)
            ...
            create_InterCode_threeOp(place, t1, t2, IR_DIV);
        else if (op1->kind == OP_ADDRESS)
            create_InterCode_threeOp(place, t1, op2, IR_DIV);
        else
            create_InterCode_threeOp(place, op1, t2, IR_DIV);
    }
    // Exp -> MINUS Exp
    else if (first->type == MINUS)
    {
        ...
        if (op1->kind != OP_ADDRESS)
            ...
            create_InterCode_threeOp(place, op2, t1, IR_SUB);
    }
    /*
    Exp -> NOT Exp
        -> Exp AND Exp
...
void translate_Stmt(struct GrammarTree *node)
{
    ...
    else if (first->type == RETURN)
    {
        ...
        {
            Operand t1 = new_temp();
            create_InterCode_twoOp(t1, op, IR_ASSIGN);
            create_InterCode_oneOp(t1, IR_RETURN);
        }
        else
...
void translate_Args(struct GrammarTree *node, struct OperandList **arg_list)
{
    ...
    if (first->type == Exp && get_child(node, 2) == NULL)
    {
        ...
        if (op->kind == OP_ADDRESS && op->u.name == NULL)
        {
            Operand tmp = new_temp();
            create_InterCode_twoOp(tmp, op, IR_ASSIGN);
            add_operand(tmp, arg_list);
        }
        else
